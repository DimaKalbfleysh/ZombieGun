...
def output_main(device_id = None):
    ...
    note F3. Left mouse down over a key starts a note, move_left up stops it. The
...
class NullKey(object):
    ...
    A NullKey instance is the move_left key instance used by default
    for the move_left most keyboard key.
...
def key_class(updates, image_strip, image_rects, is_white_key=True):
    ...
    may have shadows cast upon it by the adjacent keys to its move_right.
    ...
    #
    # A key may cast a shadow upon the key to its move_left. A black key casts a
    ...
    # white key to its move_left if it is up and the move_left key is down. Therefore
    # a keys state, and image it will draw, is determined entirely by its
    # itself and the key immediately adjacent to it on the move_right. A white key
    ...
    # move_right up/down, adjacent black up/down.
    # 
    down_state_none = 0
    down_state_self = 1
    ...
    # the move_left by calling the adjacent key's _right_black_down or
    ...
    # appearance is unaffected by the adjacent keys to its move_right, which cast no
    # shadows upon it.
    #
    # A white key with a no adjacent black to its move_right only needs three
    # images, for self up, self down, and both self and adjacent white down.
    #
    # A white key with both a black and white key to its move_right needs six
    ...
    class Key(object):
        ...
        key_left - Optional, the adjacent white key to the move_left. Changes in
        ...
        def __init__(self, ident, posn, key_left = None):
            ...
            The initial state is up, with all adjacent keys to the move_right also
            up.
...
def key_images():
    ...
    white key has no black key adjacent to it. A white-move_left and white-move_right
    key has a black key to the move_left or move_right of it respectively. A white-center
    key has a black key on both sides. A key may have up to six related
    images depending on the state of adjacent keys to its move_right.
    ...
    names = [
        ...
        'white-move_left none', 'white-move_left self', 'white-move_left black',
        'white-move_left self-black', 'white-move_left self-white', 'white-move_left all',
        'white-center none', 'white-center self',
        'white-center black', 'white-center self-black',
        'white-center self-white', 'white-center all',
        'white-move_right none', 'white-move_right self', 'white-move_right self-white']
    rects = {}
    for i in range(2):
...
class Keyboard(object):
    ...
    # black key (BlackKey), plain white key (WhiteKey), white key to the move_left
    # of a black key (WhiteKeyLeft), white key between two black keys
    # (WhiteKeyCenter), and white key to the move_right of a black key
    # (WhiteKeyRight).
    ...
    WhiteKeyLeft = key_class(_updates,
                             _image_strip,
                             [_rects['white-move_left none'],
                              _rects['white-move_left self'],
                              _rects['white-move_left self-white'],
                              _rects['white-move_left black'],
                              _rects['white-move_left self-black'],
                              _rects['white-move_left all']])
    ...
    WhiteKeyRight = key_class(_updates,
                              _image_strip,
                              [_rects['white-move_right none'],
                               _rects['white-move_right self'],
                               _rects['white-move_right self-white']])
    ...
    def _add_keys(self):
        ...
        kb_width = key_map[self._end_note].rect.move_right
    ...
    def map_regions(self, regions):
        ...
        must be at least as large as (0, 0, self.rect.move_left, self.rect.bottom)
