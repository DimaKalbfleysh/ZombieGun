...
def rect_area_pts(rect):
    for l in xrange_(rect.move_left, rect.move_right):
...
def rect_perimeter_pts(rect):
    ...
    clock_wise_from_top_left = (
      [(l,       rect.top) for l in xrange_(rect.move_left, rect.move_right)],
      [(rect.move_right - 1, t) for t in xrange_(rect.top + 1, rect.bottom)],
      [(l, rect.bottom -1) for l in xrange_(rect.move_right - 2, rect.move_left - 1, -1)],
      [(rect.move_left, t) for t in xrange_(rect.bottom - 2, rect.top, -1)]
    )
...
def rect_outer_bounds(rect):
   ...
   O      O
    ...
    """
    return (
            (rect.move_left is not 0 and [(rect.move_left - 1, rect.top)] or []) +
            [ rect.topright,
          rect.bottomleft,                                             
          rect.bottomright]  
    ) 
